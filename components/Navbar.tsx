...
const Navbar: React.FC<NavbarProps> = ({ currentPage, setCurrentPage, isDarkMode, toggleTheme }) => {
  const navLinks = [
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[60] bg-white/80 dark:bg-[#030712]/80 backdrop-blur-lg border-b border-gray-200 dark:border-gray-800 py-4">
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="md:hidden flex items-center space-x-4">
          ...
          <select 
            ...
            className="bg-gray-100 dark:bg-gray-800 text-sm rounded-lg p-1 outline-none border border-gray-300 dark:border-gray-700"
          >
            {navLinks.map(link => <option key={link.id} value={link.id}>{link.name}</option>)}
          </select>
        </div>
      </div>
    </nav>
